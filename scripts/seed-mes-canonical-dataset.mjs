...
async function rebuildBaseExecutionProjection() {
  ...
  try {
    ...
    const baseLineEligibility = baseLine ? (await master.query(`
      SELECT effective_from, effective_to, active_flag, lifecycle_status
      FROM md_production_version_line_eligibility
      WHERE production_version_id=$1 AND production_line_id=$2
      ORDER BY effective_from DESC
      LIMIT 1
    `, [pv.master_id, baseLine.master_id])).rows[0] : null;
    ...
    await execution.query(`INSERT INTO rm_routing_header (master_id, code, item_revision_id, site_id, lifecycle_status, updated_at) VALUES ($1,$2,NULL,$3,$4,NOW()) ON CONFLICT (master_id) DO UPDATE SET code=EXCLUDED.code,item_revision_id=NULL,site_id=EXCLUDED.site_id,lifecycle_status=EXCLUDED.lifecycle_status,updated_at=NOW()`, [routing.master_id, routing.code, pv.site_id, routing.lifecycle_status]);
    ...
    if (baseLine) {
      ...
      await execution.query(`INSERT INTO rm_production_version_line_eligibility (master_id, production_version_id, production_line_id, selection_role, priority, effective_from, effective_to, active_flag, lifecycle_status, updated_at) VALUES (gen_random_uuid(),$1,$2,'PRIMARY',1,$3,$4,$5,$6,NOW())`, [pv.master_id, baseLine.master_id, baseLineEligibility?.effective_from || baseLine.effective_from, baseLineEligibility?.effective_to || null, baseLineEligibility?.active_flag ?? true, baseLineEligibility?.lifecycle_status || 'Released']);
    }
