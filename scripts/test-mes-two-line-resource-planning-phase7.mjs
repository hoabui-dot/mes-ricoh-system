...
async function seedModel({ primaryComplete = true, backupComplete = false, primaryPriority = 10, backupPriority = 20 } = {}) {
  ...
  await sql(`INSERT INTO rm_mbom_header (master_id, code, name, base_quantity, base_uom_id, lifecycle_status, business_version) VALUES ($1,'P7-MBOM','{"vi":"Phase 7 MBOM"}',1,$2,'Released','1')`, [mbom, uom]);
  await sql(`INSERT INTO rm_routing_header (master_id, code, item_revision_id, site_id, lifecycle_status) VALUES ($1,'P7-RT',NULL,$2,'Released')`, [routing, site]);
