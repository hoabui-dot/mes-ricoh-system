...
func main() {
	...
	defer cancel()
	dbURL := getEnv("DATABASE_URL", "postgresql://wms_outbound_user:wms_outbound_pass@localhost:15441/wms_outbound_db")
	migrationDBURL := getEnv("MIGRATION_DATABASE_URL", dbURL)
	port := getEnv("PORT", "3090")
	brokers := strings.Split(getEnv("KAFKA_BROKERS", "localhost:9092"), ",")
	inventoryURL := getEnv("INVENTORY_SERVICE_URL", "http://wms-inventory-service:3070")
