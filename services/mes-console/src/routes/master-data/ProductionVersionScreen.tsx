  ...
  const handleReleasePV = async (pvId: string) => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        const failures = Array.isArray(errJson.failures)
          ? errJson.failures.map((failure: any) => `${failure.code || failure.rule || 'VALIDATION'}${failure.params ? ` (${Object.entries(failure.params).map(([key, value]) => `${key}=${value}`).join(', ')})` : ''}`).join('; ')
          : '';
        throw new Error(failures || errJson.message || errJson.error || t('productionVersion.releaseFailed'));
      }
        ...
        <table className="w-full text-left text-sm text-slate-300">
          ...
          <tbody className="divide-y divide-slate-800/60">
            {pvList.map((pv) => {
              ...
              const productionVersionName = text(pv.name_i18n) || text(pv.name) || text(item?.name) || text(revision?.name) || productionVersionCode;
