...
export function SkillManagementScreen() {
  ...
  const openAssignments = async (skill: Row) => {
    ...
    try {
      const [assignmentPayload, dependencyPayload] = await Promise.all([
        request(`/worker-skills/${skill.master_id}/assignments`),
        request(`/worker-skills/${skill.master_id}/dependencies`),
      ]);
      setAssignments(assignmentPayload.data || []);
      setDependencies(dependencyPayload.data || {});
    } catch (error: any) {
      toast.error(error.message);
    }
  };
  ...
  const skillColumns: BaseDataTableColumn<Row>[] = workerTab ? [
    ...
    { id: 'dependencies', header: t('skills.dependencies'), accessorFn: (row) => row.dependency_count || 0 },
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal open={skillDetailOpen} title={selectedSkill ? text(selectedSkill.name) : t('skills.selectWorkerSkill')} onClose={() => setSkillDetailOpen(false)} className="max-w-3xl">
          <p className="mb-5 text-sm text-muted-foreground">{t('skills.workerScopeHelp')}</p>
          {selectedSkill && <div className="space-y-5"><div className="grid gap-3 sm:grid-cols-4"><div className="rounded border border-border p-3"><div className="text-xs text-muted-foreground">{t('common.code')}</div><div className="font-mono text-sm">{selectedSkill.code}</div></div><div className="rounded border border-border p-3"><div className="text-xs text-muted-foreground">{t('common.level')}</div><div className="text-sm">{selectedSkill.minimum_level || 'Basic'}</div></div><div className="rounded border border-border p-3"><div className="text-xs text-muted-foreground">{t('common.status')}</div><div className="text-sm">{selectedSkill.lifecycle_status}</div></div><div className="rounded border border-border p-3"><div className="text-xs text-muted-foreground">{t('skills.scope')}</div><div className="text-sm">{selectedSkill.scope}</div></div></div><div><div className="text-sm font-semibold">{t('resourceFoundation.description')}</div><p className="mt-1 text-sm text-muted-foreground">{text(selectedSkill.description) || t('common.notAvailable')}</p></div><div className="rounded border border-border p-3"><div className="mb-2 text-sm font-semibold">{t('skills.dependencies')}</div><div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-3"><div>{t('skills.assignments')}: {dependencies?.active_assignments ?? selectedSkill.active_assignment_count ?? 0}</div><div>{t('skills.operationRequirements')}: {dependencies?.operation_skill_requirements ?? selectedSkill.operation_requirement_count ?? 0}</div><div>{t('skills.productionStandards')}: {dependencies?.production_standards ?? selectedSkill.production_standard_count ?? 0}</div></div></div><div><div className="mb-2 flex items-center justify-between gap-3"><div className="text-sm font-semibold">{t('skills.assignments')}</div><div className="text-xs text-muted-foreground">{t('skills.readOnlyAssignments')}</div></div>{assignments.length ? <div className="space-y-2">{assignments.map((assignment) => <div key={`${assignment.employee_id}-${assignment.effective_from}`} className="rounded border border-border p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><div className="font-semibold">{text(assignment.employee_name) || assignment.employee_code}</div><div className="text-xs text-muted-foreground">{assignment.employee_code} · {assignment.level} · {assignment.qualification_status} · {assignment.active_flag ? t('skills.activeAssignment') : t('skills.endedAssignment')}</div></div><Button type="button" variant="outline" size="sm" onClick={() => navigate(`/employees?employee_id=${assignment.employee_id}`)}>{t('employees.editSkills')}</Button></div></div>)}</div> : <p className="text-sm text-muted-foreground">{t('common.empty')}</p>}</div></div>}
      </Modal>
