...
export const ProductionVersionCrudScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const mbomQuery = `?limit=500&item_revision_id=${encodeURIComponent(form.item_revision_id)}&site_id=${encodeURIComponent(form.site_id)}&lifecycle_status=Released`;
    void Promise.all([fetchResource('mbom-headers', user, mbomQuery), fetchResource('routing-headers', user, '?limit=500&lifecycle_status=Released')])
