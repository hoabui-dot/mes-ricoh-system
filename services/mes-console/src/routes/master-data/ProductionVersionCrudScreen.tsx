...
export const ProductionVersionCrudScreen: React.FC = () => {
  ...
  const [form, setForm] = useState({ item_revision_id: '', mbom_header_id: '', routing_header_id: '', is_default: false, name_i18n: emptyLocalized(), min_lot_size: '', max_lot_size: '' });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (id) await putResource('production-versions', id, payload, user);
      else await postResource('production-versions', payload, user);
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><Button variant="secondary" onClick={() => navigate('/master-data/production-versions')}><ArrowLeft className="h-4 w-4" />{t('common.back')}</Button><div className="mes-page-header"><h1 className="text-xl font-bold text-foreground">{t(id ? 'productionVersion.edit' : 'productionVersion.create')}</h1><p className="text-sm text-muted-foreground">{t('productionVersion.formHelp')}</p></div><form onSubmit={submit} className="space-y-5 rounded-md border border-border bg-surface p-6"><LocalizedTextFields label={t('productionVersion.name')} value={form.name_i18n} onChange={(value: LocalizedValues) => update('name_i18n', value)} required /><div className="rounded-md border border-border bg-surface-subtle p-4"><div className="text-sm font-semibold text-foreground">{t('productionVersion.code')}</div><div className="mt-1 font-mono text-action">{id ? t('productionVersion.codeExisting') : t('productionVersion.codeGenerated')}</div></div><label className="space-y-1"><span className="text-sm text-foreground">{t('productionVersion.itemRevision')} *</span><SelectBase required value={form.item_revision_id} onValueChange={(value) => update('item_revision_id', value)} options={revisions.filter((row) => ['Released', 'InReview', 'Draft'].includes(row.lifecycle_status)).map((row) => ({ value: row.master_id, label: localizedText(row.name) || localizedText(row.item_name) || row.revision_code || row.code, secondaryLabel: row.revision_code || row.code }))} aria-label={t('productionVersion.itemRevision')} /></label><label className="space-y-1"><span className="text-sm text-foreground">{t('productionVersion.releasedMbom')} *</span><SelectBase required value={form.mbom_header_id} onValueChange={(value) => update('mbom_header_id', value)} options={matchingMboms.map((row) => ({ value: row.master_id, label: localizedText(row.name) || row.code, secondaryLabel: row.code }))} placeholder={t('productionVersion.selectMbom')} aria-label={t('productionVersion.releasedMbom')} /></label><label className="space-y-1"><span className="text-sm text-foreground">{t('productionVersion.releasedRouting')} *</span><SelectBase required value={form.routing_header_id} onValueChange={(value) => update('routing_header_id', value)} options={matchingRoutings.map((row) => ({ value: row.master_id, label: localizedText(row.name) || row.code, secondaryLabel: row.code }))} placeholder={t('productionVersion.selectRouting')} aria-label={t('productionVersion.releasedRouting')} /></label><div className="grid gap-4 sm:grid-cols-2"><label className="space-y-1"><span className="text-sm text-foreground">{t('productionVersion.minLotSize')}</span><input type="number" min="0" step="any" value={form.min_lot_size} onChange={(event) => update('min_lot_size', event.target.value)} className="w-full rounded-md border border-border bg-background p-3 text-foreground" /></label><label className="space-y-1"><span className="text-sm text-foreground">{t('productionVersion.maxLotSize')}</span><input type="number" min="0" step="any" value={form.max_lot_size} onChange={(event) => update('max_lot_size', event.target.value)} className="w-full rounded-md border border-border bg-background p-3 text-foreground" /></label></div>{matchingMboms.length === 0 && <p className="text-sm text-amber-400">{t('productionVersion.noMatchingMbom')}</p>}<label className="flex items-center gap-2 text-sm text-foreground"><input type="checkbox" checked={form.is_default} onChange={(event) => update('is_default', event.target.checked)} />{t('productionVersion.defaultConfiguration')}</label><div className="flex justify-end"><Button type="submit" disabled={saving || !form.name_i18n.vi.trim() || !form.mbom_header_id || !form.routing_header_id}><Save className="h-4 w-4" />{t('common.save')}</Button></div></form></div>;
};
