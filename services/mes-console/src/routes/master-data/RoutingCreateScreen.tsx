...
export const RoutingCreateScreen: React.FC = () => {
  ...
  const [operations, setOperations] = useState<any[]>([]); const [revisions, setRevisions] = useState<any[]>([]); const [supported, setSupported] = useState<Record<string, SupportedWorkCenter[]>>({});
  const [saving, setSaving] = useState(false); const [codePreview, setCodePreview] = useState('');
  const [form, setForm] = useState({ code: '', item_revision_id: '', name: emptyLocalized(), description: emptyLocalized(), business_version: '1', routing_type: 'Standard', production_purpose: emptyLocalized(), effective_from: new Date().toISOString().slice(0, 10), effective_to: '', engineering_note: emptyLocalized(), reference_document: '' });
  const [flow, setFlow] = useState<FlowRow[]>([]);

  useEffect(() => {
    void (async () => {
      try {
        const [operationRows, revisionRows, workCenterRows, previewResponse] = await Promise.all([fetchResource('operations', user), fetchResource('item-revisions', user, '?limit=500&lifecycle_status=Released'), fetchResource('work-centers', user), id ? Promise.resolve(null) : fetch(`${masterDataBaseUrl()}/routing-headers/code-preview`, { headers: authHeaders(user) })]);
        setRevisions(revisionRows.filter((row: any) => ['FG', 'SFG'].includes(row.item_type || row.item_group)));
        if (!id && revisionRows[0]) setForm((current) => ({ ...current, item_revision_id: current.item_revision_id || revisionRows[0].master_id }));
        ...
        if (id) {
          ...
          setForm({ code: current.code || '', item_revision_id: current.item_revision_id || '', name: current.name || emptyLocalized(), description: current.description || emptyLocalized(), business_version: current.business_version || String(current.version_no || 1), routing_type: current.routing_type || 'Standard', production_purpose: current.production_purpose || emptyLocalized(), effective_from: String(current.effective_from || '').slice(0, 10), effective_to: current.effective_to ? String(current.effective_to).slice(0, 10) : '', engineering_note: current.engineering_note || emptyLocalized(), reference_document: current.reference_document || '' });
  ...
  const selectOperation = (index: number, operationId: string) => updateFlow(index, { operation_id: operationId, work_center_id: '' });
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <form onSubmit={submit} className="space-y-6">
      <section className="space-y-4 rounded-md border border-border bg-surface p-6"><h2 className="text-base font-bold text-foreground">{t('routing.section.basic')}</h2><div className="grid gap-4 sm:grid-cols-2">
        <label className="space-y-1"><span className="text-sm text-foreground">{t('routing.code')} *</span><input readOnly required value={codePreview} className="w-full cursor-not-allowed rounded-md border border-border bg-surface-subtle p-3 font-mono text-action" /><span className="text-xs text-muted-foreground">{t('routing.codePreviewHelp')}</span></label>
        <label className="space-y-1"><span className="text-sm text-foreground">{t('routing.outputRevision')} *</span><SelectBase required value={form.item_revision_id} onValueChange={(value) => updateForm('item_revision_id', value)} options={revisions.map((row) => ({ value: row.master_id, label: text(row.item_name) || text(row.name) || row.revision_code, secondaryLabel: `${row.item_code || ''} · ${row.revision_code || ''}` }))} aria-label={t('routing.outputRevision')} /></label>
        <label className="space-y-1"><span className="text-sm text-foreground">{t('routing.version')} *</span><input required value={form.business_version} onChange={(event) => updateForm('business_version', event.target.value)} className="w-full rounded-md border border-border bg-background p-3 text-foreground" /></label>
        <label className="space-y-1"><span className="text-sm text-foreground">{t('routing.type')} *</span><SelectBase required value={form.routing_type} onValueChange={(value) => updateForm('routing_type', value)} options={['Standard', 'Alternate', 'Rework'].map((value) => ({ value, label: t(`routing.type.${value}`) }))} aria-label={t('routing.type')} /></label>
        <LocalizedTextFields label={t('routing.name')} value={form.name} onChange={(value: LocalizedValues) => updateForm('name', value)} required /><LocalizedTextFields label={t('routing.description')} value={form.description} onChange={(value: LocalizedValues) => updateForm('description', value)} multiline />
      </div></section>
      <section className="space-y-4 rounded-md border border-border bg-surface p-6"><h2 className="text-base font-bold text-foreground">{t('routing.section.engineering')}</h2><div className="grid gap-4 sm:grid-cols-2">
        <LocalizedTextFields label={t('routing.productionPurpose')} value={form.production_purpose} onChange={(value: LocalizedValues) => updateForm('production_purpose', value)} multiline />
        <label className="space-y-1"><span className="text-sm text-foreground">{t('routing.validFrom')} *</span><input required type="date" value={form.effective_from} onChange={(event) => updateForm('effective_from', event.target.value)} className="w-full rounded-md border border-border bg-background p-3 text-foreground" /></label>
        <label className="space-y-1"><span className="text-sm text-foreground">{t('routing.validTo')}</span><input type="date" value={form.effective_to} onChange={(event) => updateForm('effective_to', event.target.value)} className="w-full rounded-md border border-border bg-background p-3 text-foreground" /></label>
        <LocalizedTextFields label={t('routing.engineeringNote')} value={form.engineering_note} onChange={(value: LocalizedValues) => updateForm('engineering_note', value)} multiline /><label className="space-y-1 sm:col-span-2"><span className="text-sm text-foreground">{t('routing.referenceDocument')}</span><input value={form.reference_document} onChange={(event) => updateForm('reference_document', event.target.value)} className="w-full rounded-md border border-border bg-background p-3 text-foreground" /></label>
      </div></section>
      <section className="space-y-4 rounded-md border border-border bg-surface p-6"><div><h2 className="text-base font-bold text-foreground">{t('routing.operationFlow')}</h2><p className="text-sm text-muted-foreground">{t('routing.operationFlowHelp')}</p></div>
        {flow.map((row, index) => { return <div key={index} className="grid gap-3 rounded-md border border-border bg-surface-subtle p-4 sm:grid-cols-[100px_1.2fr_1.5fr_140px_auto]">
          ...
          <label className="space-y-1"><span className="text-xs text-muted-foreground">{t('routing.predecessor')}</span><input type="number" min="1" value={row.predecessor_seq} onChange={(event) => updateFlow(index, { predecessor_seq: event.target.value })} className="w-full rounded-md border border-border bg-background p-2 text-foreground" /></label>
