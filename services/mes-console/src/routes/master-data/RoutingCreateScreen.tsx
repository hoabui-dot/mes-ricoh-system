...
export const RoutingCreateScreen: React.FC = () => {
  ...
  useEffect(() => {
    void Promise.all([fetchResource('operations', user), fetch(`${masterDataBaseUrl()}/routing-headers/code-preview`, { headers: authHeaders(user) })])
      .then(async ([operationRows, previewResponse]) => { setOperations(operationRows.filter((row: any) => !['Inactive', 'Obsolete'].includes(row.lifecycle_status))); const preview = await previewResponse.json(); if (!previewResponse.ok) throw new Error(preview.message || preview.error); setCodePreview(preview.preview_code || ''); })
  ...
  const loadSupported = async (operationId: string): Promise<SupportedWorkCenter[]> => {
    if (!operationId || supported[operationId]) return supported[operationId] || [];
    const response = await fetch(`${masterDataBaseUrl()}/operations/${operationId}/supported-work-centers`, { headers: authHeaders(user) });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const created = await postResource('routing-headers', { ...form, code: codePreview, effective_to: form.effective_to || null }, user);
      const routingId = created.master_id || created.data?.master_id;
      for (const [index, row] of flow.entries()) {
        const operation = operations.find((item) => item.master_id === row.operation_id);
        await postResource('routing-operations', { code: `${codePreview}-${String(row.seq || (index + 1) * 10).padStart(3, '0')}`, name: operation?.name || { vi: operation?.code || 'Operation' }, routing_header_id: routingId, operation_id: row.operation_id, work_center_id: row.work_center_id, seq: Number(row.seq), predecessor_seq: row.predecessor_seq ? Number(row.predecessor_seq) : null }, user);
      }
      toast.success(t('routing.createSuccess')); navigate(`/master-data/routings/${routingId}/operations`);
