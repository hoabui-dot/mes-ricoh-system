...
export const MbomCreateScreen: React.FC = () => {
  ...
  const [form, setForm] = useState({
    ...
    site_id: '', business_version: '1', purpose: 'Standard', base_quantity: '100',
  ...
  useEffect(() => {
    void Promise.all([fetchResource('sites', user), fetchResource('uoms', user)])
      .then(([siteRows, uomRows]) => {
        setSites(siteRows);
        setUoms(uomRows);
        setForm((current) => ({ ...current, site_id: current.site_id || siteRows[0]?.master_id || '', base_uom_id: current.base_uom_id || uomRows[0]?.master_id || '' }));
