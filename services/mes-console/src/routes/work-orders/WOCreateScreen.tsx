    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-md p-6 shadow-2xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><FieldLabel t={t} labelKey="woCreate.quantity" helpKey="workOrders.create.fieldHelp.quantity" required /><div className="flex items-center gap-2"><input required min={1} type="number" value={quantity} onChange={(e) => setQuantity(Number(e.target.value))} className="w-full bg-slate-950 border border-slate-800 rounded-md p-3.5 font-mono text-sm text-slate-100 focus:outline-none focus:border-action" /><span className="font-mono text-xs text-slate-400">{selectedProduct?.base_uom_code || 'UOM'}</span></div></div><div><FieldLabel t={t} labelKey="woCreate.targetDate" helpKey="workOrders.create.fieldHelp.targetDate" required /><input required type="date" value={targetDate} onChange={(e) => setTargetDate(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-md p-3.5 font-mono text-sm text-slate-100 focus:outline-none focus:border-action" /></div></div>
          ...
          <div className="pt-4 flex justify-end"><Button type="submit" disabled={submitting || !selectedProduct || !shiftId || !Number.isFinite(quantity) || quantity <= 0 || (selectedProduct?.min_lot_size != null && quantity < Number(selectedProduct.min_lot_size)) || (selectedProduct?.max_lot_size != null && quantity > Number(selectedProduct.max_lot_size))}><CheckCircle className="h-4 w-4" />{t('woCreate.submit')}</Button></div>
        </form>
      </div>
