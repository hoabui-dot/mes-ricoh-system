...
export const WODetailScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedOperation && <div className="bg-slate-900 border border-action/50 rounded-md p-6 space-y-4"><div className="flex items-center justify-between"><div><h3 className="text-base font-bold text-slate-100">{localizedText(selectedOperation.operation_name) || selectedOperation.operation_code}</h3><p className="text-xs text-slate-400">{t('woDetail.resourceCandidatesSubtitle')}</p></div><button type="button" onClick={() => setSelectedOperation(null)} className="text-xs text-slate-400 hover:text-white">{t('common.cancel')}</button></div>{loadingCandidates ? <Loader2 className="h-5 w-5 animate-spin text-action" /> : candidates.length === 0 ? <>{candidateBlockers.map((blocker: any) => <p key={blocker.code} className="text-sm text-rose-300">{translateWorkOrderError(blocker.code, t) || blocker.message || blocker.code}</p>)}<p className="text-sm text-slate-400">{t('woDetail.resourceNoCandidates')}</p></> : <div className="grid gap-3 md:grid-cols-2">{candidates.map((candidate, index) => <div key={`${candidate.machine_group?.id || candidate.equipment?.id || candidate.workstation?.id}-${index}`} className="rounded-md border border-slate-800 bg-slate-950 p-4"><div className="flex items-center justify-between"><div><span className="text-xs font-bold text-amber-300">#{index + 1}</span><h4 className="font-semibold text-slate-100">{localizedText(candidate.machine_group?.name) || localizedText(candidate.equipment?.name) || candidate.equipment?.code || localizedText(candidate.workstation?.name) || candidate.workstation?.code || t('common.notAvailable')}</h4><p className="text-xs text-slate-500">{t('woDetail.resourceMachineGroup')}: {candidate.machine_group?.code || candidate.equipment?.code || candidate.workstation?.code || t('common.notAvailable')}</p></div><span className={candidate.readiness === 'Eligible' ? 'text-emerald-300' : candidate.readiness === 'ReadyWithWarnings' ? 'text-amber-300' : 'text-rose-300'}>{t('woDetail.resourceReadiness')}: {candidate.readiness}</span></div>{candidate.primary_machine && <div className="mt-2 text-xs text-slate-300">{t('resourceFoundation.primaryMachine')}: {localizedText(candidate.primary_machine.name) || candidate.primary_machine.code} · {candidate.primary_machine.unit_code || ''}</div>}{candidate.supporting_machines?.length ? <div className="mt-1 text-xs text-slate-400">{t('resourceFoundation.supportingMachines')}: {candidate.supporting_machines.map((member: any) => member.code).join(', ')}</div> : null}<div className="mt-3 space-y-1 text-xs text-slate-400"><div>{t('woDetail.resourceDuration')}: {candidate.estimated_duration_min || candidate.calculation?.estimated_duration_min || '-'} min</div><div>{t('woDetail.resourceCapacity')}: {candidate.calendar?.available_minutes || '-'} min</div>{(candidate.blocking_errors || []).map((blocker: any) => <div key={blocker.code} className="text-rose-300">{translateWorkOrderError(blocker.code, t) || blocker.message || blocker.code}</div>)}{(candidate.warnings || []).map((warning: any) => <div key={warning.code} className="text-amber-300">{translateWorkOrderError(warning.code, t) || warning.message || warning.code}</div>)}{(candidate.capacity_conflicts || []).map((conflict: any) => <div key={conflict.code} className="text-rose-300">{translateWorkOrderError(conflict.code, t) || conflict.message || conflict.code}</div>)}</div><button type="button" disabled={allocating || candidate.readiness === 'Blocked' || (candidate.capacity_conflicts || []).length > 0} onClick={() => commitCandidate(candidate)} className="mt-4 inline-flex items-center gap-2 rounded-md bg-action px-3 py-2 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Check className="h-4 w-4" />{t('woDetail.resourceSelect')}</button></div>)}</div>}</div>}
            ...
            <tbody className="divide-y divide-slate-800/60">
              {(wo.operations || [
                ...
                <tr key={idx} className="hover:bg-slate-800/40 font-mono text-xs">
                  ...
                  <td className="px-5 py-3 text-right text-slate-200">{op.expected_good_quantity ?? wo.quantity} <span className="text-xs text-slate-500">/ {op.base_quantity ?? op.standard_base_quantity ?? '-'}</span></td>
