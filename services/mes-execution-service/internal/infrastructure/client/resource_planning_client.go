...
type ResourcePlanningClient struct {
	baseURL string
	http    *http.Client
	cb      interface{ Execute(func() (interface{}, error)) (interface{}, error) }
}
...
func (c *ResourcePlanningClient) Readiness(ctx context.Context, body map[string]interface{}, headers map[string]string) (map[string]interface{}, error) {
	result, err := c.cb.Execute(func() (interface{}, error) {
		...
		if err != nil { return nil, sharedkernel.NewRetryableDependencyError("mes-master-data-service", err) }
		defer resp.Body.Close()
		var payload map[string]interface{}
		if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil { return nil, err }
		if resp.StatusCode >= 500 { return nil, sharedkernel.NewRetryableDependencyError("mes-master-data-service", fmt.Errorf("readiness failed: %s", resp.Status)) }
		if resp.StatusCode >= 400 { return nil, fmt.Errorf("readiness rejected: %s", resp.Status) }
		return payload, nil
	})
	if err != nil { return nil, err }
	payload, ok := result.(map[string]interface{}); if !ok { return nil, fmt.Errorf("invalid readiness response") }
	return payload, nil
}
