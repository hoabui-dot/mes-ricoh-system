...
///   - Nacks with requeue on transient handler failure so durable inbox logic can retry.
...
public sealed class RabbitMqConsumer : IRabbitMqConsumer, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        consumer.ReceivedAsync += async (_, args) =>
        {
            ...
            {
                _logger.LogError(ex,
                    "Error processing RabbitMQ message. routingKey={RoutingKey} — nacking for retry",
                    routingKey);

                // Requeue transaction/infrastructure failures so durable inbox handling can retry safely.
                // Payload validation handlers return successfully after recording a non-retryable error.
    ...
    private async Task EnsureConnectedAsync(CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogWarning(ex, "RabbitMQ unavailable; consumer connection will retry in 5 seconds");
