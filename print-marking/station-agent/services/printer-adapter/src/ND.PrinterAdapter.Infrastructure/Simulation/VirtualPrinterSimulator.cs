...
/// Publishes an immediate Kafka heartbeat on connect/disconnect so the kiosk
        ...
        foreach (var ep in _endpoints)
            if (printerCode == null || ep.PrinterCode.Equals(printerCode, StringComparison.OrdinalIgnoreCase))
            {
                ...
                try
                {
                    ...
                    await _publisher.PublishAsync(_kafkaOptions.DeviceHeartbeatsTopic, ep.PrinterCode, JsonSerializer.Serialize(hb), ct);
