...
public sealed class JobProcessingConsumer : BackgroundService
{
    ...
    private const string ConsumerGroup = "printer-adapter-job-processing-v1";
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        await _consumer.ConsumeAsync(
            ...
            async (json, ct) =>
            {
                var eventType = JsonSerializer.Deserialize<JobEventBase>(json, JsonSerializerOptions)?.EventType;
                if (string.Equals(eventType, "JobProcessing", StringComparison.OrdinalIgnoreCase))
    ...
    private async Task HandleMessageAsync(string payloadJson, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await _publisher.PublishAsync(_kafkaOptions.JobEventsTopic, evt.JobId, eventJson, cancellationToken);
