...
/// <summary>
/// Kafka request/reply boundary for remote Printer Adapter management.
/// Projection Service must not depend on the adapter's HTTP listener.
/// </summary>
public sealed class PrinterManagementKafkaClient : BackgroundService
{
    ...
    private const string ConsumerQueue = "projection-service.printer-management-responses";
    ...
    public async Task<PrinterManagementResponse> RequestAsync(string method, string path, string? query, string body, string requestedBy, CancellationToken ct)
    {
        ...
        try
        {
            await _publisher.PublishAsync(Exchange, RequestKey, JsonSerializer.Serialize(new
            {
                event_id = requestId,
